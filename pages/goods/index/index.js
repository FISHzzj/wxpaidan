...
Page({
    ...
    bindSearch: function(t) {
        ...
        var searchname = a.trim(t.detail.value);
        var o = getApp();
        var q = o.requirejs("core");
        q.post("shop/wind_type", {
            name:searchname
        }, function(res) {
          console.log(res)
            if(res.status == 0){
                q.alert(res.result.message);
            }else{
                var appid = res.result.info.appid;
                var spotid = res.result.info.spotid;
                
                wx.navigateTo({
                  url: `/pages/goods/windy/index?appid=${appid}&spotid=${spotid}`,
                })
